...
func (a *Adapter) PushAvailability(ctx context.Context, updates []domain.AvailabilityUpdate) error {
	...
	req.Header.Set("x-ota-provider", "bookingcom")
	
	resp, err := a.client.Do(req)
	if err != nil {
		return fmt.Errorf("booking.com mock ota error: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode >= 400 {
		return fmt.Errorf("booking.com mock ota returned status %d", resp.StatusCode)
	}
	return nil
}
...
func (a *Adapter) FetchReservations(ctx context.Context, propertyID string, since time.Time) ([]domain.FetchedReservation, error) {
	req, _ := http.NewRequestWithContext(ctx, "GET", "http://localhost:3001/api/reservations?provider=bookingcom", nil)
	resp, err := a.client.Do(req)
	if err != nil {
		return nil, fmt.Errorf("booking.com mock ota fetch error: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("booking.com mock ota returned status %d", resp.StatusCode)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil {
		return nil, fmt.Errorf("booking.com decode error: %w", err)
	}
